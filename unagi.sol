...
// File: contracts/protocols/Unagi.sol

pragma solidity >=0.6.8;
pragma experimental ABIEncoderV2;


contract UnagiToken is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "Unagi Token";
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_holderUnagi).div(
            10 ** 2
        );
    }
    ...
    function removeAllFee() private {
        if (_holderUnagi == 0 && _liquidityFee == 0) return;

        _previousUnagiHolding = _holderUnagi;
    ...
    function restoreAllFee() private {
        _holderUnagi = _previousUnagiHolding;
    ...
    // Innovation for protocol by Unagi Team
    uint256 public _maxTxAmount = _tTotal; // should be 0.01% percent per transaction
    uint256 public disruptiveCoverageFee = 100 ether; // antiwhale
    mapping(address => uint256) public nextAvailableClaimDate;
    bool public swapAndLiquifyEnabled = false; // should be true
    uint256 public disruptiveTransferEnabledFrom = 0;

    uint256 public _holderUnagi = 7;
    uint256 private _previousUnagiHolding = _holderUnagi;

    uint256 public _liquidityFee = 3; // 3% will be added pool
    ...
    function activateContract() public onlyOwner {
        // protocol
        disruptiveCoverageFee = 100 ether;
        disruptiveTransferEnabledFrom = block.timestamp;
        setMaxTxPercent(10000); // 0.01% per transaction
